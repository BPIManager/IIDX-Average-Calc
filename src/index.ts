...
const init = async () => {
  ...
  Object.keys(res).map(async (song: string) => {
    ...
    if (songName === "Blind Justice ～Torn souls, Hurt Faiths ～[A]")
      songName = "Blind Justice ～Torn souls， Hurt Faiths ～[A]";
    if (songName === "COLOSSEUM [A]") songName = "COLOSSEUM[A]";
    if (songName === "Erosion Mark  [A]") songName = "Erosion Mark[A]";
    if (songName === "ROCK女 feat. 大山愛未, Ken[A]")
      songName = "ROCK女 feat. 大山愛未， Ken[A]";

    let finalSongName = nameReplacer(songName);
    if (finalSongName === "ZENITH[A]") {
      console.log(JSON.stringify(rank.sort((a, b) => b - a)));
    }
    await write(
      outputDir + finalSongName + ".json",
      JSON.stringify(rank.sort((a, b) => b - a))
    );
  });
};
...
const readMax = async () => {
  ...
  return m.reduce((group: any, item: any) => {
    ...
    if (!item) return group;
    const d =
      item.difficulty === "3" ? "[H]" : item.difficulty === "4" ? "[A]" : "[L]";
...
const rs = async () => {
  ...
  for (let i = 0; i < files.length; ++i) {
    const f = files[i];
    let songName = f.replace(".json", "").replace("&amp;", "&");
    const fullPath = outputDir + f;
    const r = await openFile(fullPath);
    const wr = wrs[songName];
    const max =
      maxes[
        songName === "Rave*it!! Rave*it!!" ? "Rave*it!! Rave*it!! " : songName
      ] || 0;
    if (!wr) {
    ...
    res.push({
      title: nameReplacer(songName, true),
      difficulty: _diff,
      wr: wrLogic(),
