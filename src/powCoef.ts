...
const loadCSV = async () => {
  ...
  for (let i = 0; i < split.length; ++i) {
    ...
    const difficulty = title.indexOf("[A]") > 0 ? "4" : title.indexOf("[L]") > 0 ? "10" : "3";
...
const init = async () => {
  ...
  for (let i = 0; i < origData.length; ++i) {
    ...
    const target = newData.find((item) => item.title === current.title && item.difficulty === current.difficulty);
    const m = current.difficulty === "4" ? "A" : current.difficulty === "10" ? "L" : "H";
    const newAvg = newAvgs.find((item: any) => item.title === (current.title + "[" + m + "]"));
